...
def make_tuning_step(model, optimizer, target_representation, should_reconstruct_content, content_feature_maps_index, style_feature_maps_indices):
    ...
    def tuning_step(optimizing_img):
        ...
        # Computes gradients
        loss.backward()
        # Updates parameters and zeroes gradients parameters in this case is the image that we are updating
        optimizer.step()
        optimizer.zero_grad()
        # Returns the loss
...
def reconstruct_image_from_representation(config):
    ...
    # The features maps and gram matrices are for the inital image can be content image from content folder or style image from style folder
    ########################################################################################################################################

    def generator_and_plotter(set_of_feature_maps,name):
        if should_reconstruct_content:
            global target_content_representation
            ...
            if should_visualize_representation:
                ...
                for i in range(num_of_feature_maps):
                    ...
                    plt.show()
                    filename = f'fm_{config["model"]}_{content_feature_maps_index_name[1]}_{name}_{str(i).zfill(config["img_format"][0])}{config["img_format"][1]}_{name}'
                    utils.save_image(feature_map, os.path.join(dump_path, filename))
        else:
            global target_style_representation
            ...
            if should_visualize_representation:
                ...
                for i in range(num_of_gram_matrices):
                    Gram_matrix = target_style_representation[i].squeeze(axis=0).to('cpu').detach().numpy()
                    ...
                    plt.show()
                    filename = f'gram_{config["model"]}_{style_feature_maps_indices_names[1][i]}_{name}_{str(i).zfill(config["img_format"][0])}{config["img_format"][1]}'
                    utils.save_image(Gram_matrix, os.path.join(dump_path, filename))

    generator_and_plotter(set_of_feature_maps,'inital_img')
    ...
    ############################################################################################################################################################################
    # Plotting the style gram matrices and feature maps after the optimizing image is updated and this for the optimizing image which started as a noise image or gaussian noise
    ############################################################################################################################################################################

    set_of_feature_maps = neural_net(optimizing_img)
    generator_and_plotter(set_of_feature_maps,'final_optimized_img')
    return dump_path
