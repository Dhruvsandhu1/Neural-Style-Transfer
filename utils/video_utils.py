...
def create_video_from_intermediate_results(results_path, img_format):
    ...
    # Detect the first frame number based on file names
    files = sorted([f for f in os.listdir(results_path) if f.endswith(img_format[1])])
    if not files:
        print("No images found in the results path.")
        return
    first_frame = int(files[0].split('.')[0])  # assumes files like 0001.png
    number_of_frames_to_process = len(files)

    ffmpeg = 'ffmpeg'
    if shutil.which(ffmpeg):
        img_name_format = '%' + str(img_format[0]) + 'd' + img_format[1]  # e.g., '%4d.png'
        pattern = os.path.join(results_path, img_name_format).replace("\\", "/")
        out_video_path = os.path.join(results_path, out_file_name)
        ...
        print([ffmpeg, *input_options, *trim_video_command, *encoding_options, out_video_path])

        subprocess.run([ffmpeg, *input_options, *trim_video_command, *encoding_options, out_video_path], check=True)
    else:
        print(f'{ffmpeg} not found in the system path, aborting.')